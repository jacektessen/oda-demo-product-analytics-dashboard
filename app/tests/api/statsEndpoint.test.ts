...
test.describe("Stats Endpoint", () => {
  ...
  test("should have valid cache info", async ({ request }) => {
    ...
    expect(cacheInfo).toHaveProperty("next_update_at");

    expect(typeof cacheInfo.ttl_seconds).toBe("number");
    expect(typeof cacheInfo.next_update_at).toBe("string");
    ...
    // next_update_at should be less than ttl_seconds
    const nextUpdate = new Date(cacheInfo.next_update_at);
    const currentTime = new Date();
    const differenceInSeconds = (nextUpdate.getTime() - currentTime.getTime()) / 1000;

    expect(differenceInSeconds).toBeLessThan(cacheInfo.ttl_seconds);
  });
